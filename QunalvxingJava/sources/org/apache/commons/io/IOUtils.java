...
public class IOUtils {
    ...
    public static long copyLarge(Reader reader, Writer writer, long j, long j2, char[] cArr) {
        ...
        while (i > 0) {
            ...
            j3 += (long) read;
            if (j2 > 0) {
                i = (int) Math.min(j2 - j3, (long) cArr.length);
            }
        }
        return j3;
    }
    ...
    public static boolean contentEquals(Reader reader, Reader reader2) {
        ...
        for (int read = toBufferedReader.read(); -1 != read; read = toBufferedReader.read()) {
            if (read != toBufferedReader2.read()) {
                return false;
            }
        }
        if (toBufferedReader2.read() == -1) {
            return true;
        }
        return false;
    }
