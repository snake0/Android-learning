...
public class GridLayout extends ViewGroup {
    ...
    class Bounds {
        ...
        public final void include(GridLayout gridLayout, View view, Spec spec, Axis axis, int i) {
            ...
            int alignmentValue = gridLayout.getAlignment(spec.alignment, axis.horizontal).getAlignmentValue(view, i, ViewGroupCompat.getLayoutMode(gridLayout));
    ...
    final class Axis {
        ...
        /* synthetic */ Axis(GridLayout gridLayout, boolean z, AnonymousClass1 anonymousClass1) {
            this(z);
        }
        ...
        private void computeGroupBounds() {
            ...
            for (int i = 0; i < childCount; i++) {
                ...
                Spec spec = this.horizontal ? layoutParams.columnSpec : layoutParams.rowSpec;
                ((Bounds) this.groupBounds.getValue(i)).include(GridLayout.this, childAt, spec, this, spec.weight == 0.0f ? GridLayout.this.getMeasurementIncludingMargin(childAt, this.horizontal) : getOriginalMeasurements()[i] + getDeltas()[i]);
            }
        }
        ...
        private String arcsToString(List<Arc> list) {
            String str = this.horizontal ? MapViewConstants.ATTR_X : MapViewConstants.ATTR_Y;
        ...
        private void shareOutDelta() {
            ...
            while (i2 < childCount) {
                ...
                layoutParams = GridLayout.this.getLayoutParams(childAt);
                f = (this.horizontal ? layoutParams.columnSpec : layoutParams.rowSpec).weight;
                if (f != 0.0f) {
                    i3 += GridLayout.this.getMeasurement(childAt, this.horizontal) - getOriginalMeasurements()[i2];
                    f += f2;
                } else {
                    f = f2;
                }
            ...
            i2 = GridLayout.this.getChildCount();
            while (i < i2) {
                layoutParams = GridLayout.this.getLayoutParams(GridLayout.this.getChildAt(i));
                f = (this.horizontal ? layoutParams.columnSpec : layoutParams.rowSpec).weight;
                if (f != 0.0f) {
                    childCount = Math.round((((float) i3) * f) / f2);
                    this.deltas[i] = childCount;
                    i3 -= childCount;
                    f = f2 - f;
                } else {
                    f = f2;
    ...
    public class LayoutParams extends MarginLayoutParams {
        ...
        private void init(Context context, AttributeSet attributeSet) {
            ...
            try {
                ...
                this.columnSpec = GridLayout.spec(obtainStyledAttributes.getInt(COLUMN, Integer.MIN_VALUE), obtainStyledAttributes.getInt(COLUMN_SPAN, DEFAULT_SPAN_SIZE), GridLayout.getAlignment(i, (boolean) GridLayout.DEFAULT_ORDER_PRESERVED), obtainStyledAttributes.getFloat(COLUMN_WEIGHT, 0.0f));
    ...
    private int getDefaultMargin(View view, LayoutParams layoutParams, boolean z, boolean z2) {
        ...
        boolean z3 = i2 != 0 ? interval.min == 0 ? DEFAULT_ORDER_PRESERVED : false : interval.max == axis.getCount() ? DEFAULT_ORDER_PRESERVED : false;
    ...
    public void onMeasure(int i, int i2) {
        ...
        setMeasuredDimension(ViewCompat.resolveSizeAndState(Math.max(measure + paddingLeft, getSuggestedMinimumWidth()), i, 0), ViewCompat.resolveSizeAndState(Math.max(measure2 + paddingTop, getSuggestedMinimumHeight()), i2, 0));
    }
    ...
    /* Access modifiers changed, original: final */
    public final Alignment getAlignment(Alignment alignment, boolean z) {
        if (alignment != UNDEFINED_ALIGNMENT) {
            return alignment;
        }
        return z ? START : BASELINE;
    }
        ...
        while (true) {
            ...
            if (i7 < childCount) {
                ...
                if (childAt.getVisibility() != 8) {
                    ...
                    paddingBottom = bounds2.getOffset(this, childAt, alignment2, measurement2 + margin4, false);
